...
def train_epoch(
    ...
    for cur_iter, (data,semantic,labels, index, time, meta) in enumerate(
        train_loader
    ):
        ...
        if cfg.NUM_GPUS:
            ...
            #    semantic[i] = semantic[i].cuda(non_blocking=True)
            labels = labels.cuda()
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                if cfg.MVIT.PRETRAINING != True:
                    preds, dist_loss = model(inputs,labels,semantic=semantic)
                else:
                    loss = model(inputs,semantic=semantic)
        ...
        if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
            ...
            # Compute the loss.
            loss = loss_fun(preds.float(), labels.float()) + dist_loss
...
def build_trainer(cfg):
    ...
    val_seen_loader = loader.construct_loader(cfg, "val_seen")
    test_seen_loader = loader.construct_loader(cfg, "test_seen")
    val_unseen_loader = loader.construct_loader(cfg, "val_seen")
    test_unseen_loader = loader.construct_loader(cfg, "test_seen")
    precise_bn_loader = loader.construct_loader(
        cfg, "train", is_precise_bn=True
    )
    # Create meters.
    train_meter = TrainMeter(len(train_loader), cfg)
    val_seen_meter = ValMeter(len(val_seen_loader), cfg)
    test_seen_meter = ValMeter(len(test_seen_loader), cfg)
    val_unseen_meter = ValMeter(len(val_unseen_loader), cfg)
    test_unseen_meter = ValMeter(len(test_unseen_loader), cfg)
    return (
        model,
        optimizer,
        train_loader,
...
def train(cfg):
    ...
    val_seen_loader = loader.construct_loader(cfg, "val_seen")
    test_seen_loader = loader.construct_loader(cfg, "test_seen")
    val_unseen_loader = loader.construct_loader(cfg, "val_unseen")
    test_unseen_loader = loader.construct_loader(cfg, "test_unseen")
    precise_bn_loader = (
    ...
    if cfg.DETECTION.ENABLE:
        ...
        val_seen_meter = AVAMeter(len(val_seen_loader), cfg, mode="val_seen")
        val_unseen_meter = AVAMeter(len(val_unseen_loader), cfg, mode="val_unseen")
        test_seen_meter = AVAMeter(len(test_seen_loader), cfg, mode="test_seen")
        test_unseen_meter = AVAMeter(len(test_unseen_loader), cfg, mode="test_unseen")
    else:
        train_meter = TrainMeter(len(train_loader), cfg)
        val_seen_meter = ValMeter(len(val_seen_loader), cfg)
        test_seen_meter = ValMeter(len(test_seen_loader), cfg)
    ...
    if writer is not None:
        writer.close()
    result_string = "Val Top1 Acc: {:.2f} Top5 Acc: {:.2f} MEM: {:.2f}" "".format(
        100 - val_seen_meter.min_top1_err,
        100 - val_seen_meter.min_top5_err,
        misc.gpu_mem_usage(),
    )
    result_string = "Test Top1 Acc: {:.2f} Top5 Acc: {:.2f} MEM: {:.2f}" "".format(
        100 - test_seen_meter.min_top1_err,
        100 - test_seen_meter.min_top5_err,
        misc.gpu_mem_usage(),
    )
