...
def multiple_samples_collate(batch, fold=False):
    ...
    """
    inputs, semantic, labels, video_idx, time, extra_data = zip(*batch)
    semantic = [item for sublist in semantic for item in sublist]
    ...
    #print(labels)s
    if len(video_idx) < 1:
        video_idx = [torch.Tensor(1000)]
    inputs, semantic, labels, video_idx, time, extra_data = (
        default_collate(inputs),
        default_collate(semantic),
    ...
    if fold:
        return [inputs], semantic, labels, video_idx, time, extra_data
    else:
        return inputs, semantic, labels, video_idx, time, extra_data
...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    Args:
        ...
        split (str): the split of the data loader. Options include `train`,
            `val`, and `test`.
    """
    #assert split in ["train", "val_seen","val_unseen", "test_seen", "test_unseen"]
    if split in ["train"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = True
        drop_last = True
    elif split in ["val_seen", "val_unseen", "gradcam_unseen", "use_case"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = False
    elif split in ["test_seen", "test_unseen", "gradcam_seen"]:
