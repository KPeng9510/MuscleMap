...
def pyav_decode_optical_flow(
    ...
    # videos does not support fetching the decoding information, for that case
    # it will get None duration.
    fps = float(container_x.streams.video[0].average_rate)
    frames_length = container_x.streams.video[0].frames
    duration = container_x.streams.video[0].duration
    num_frames = num_frames[0]
    sampling_rate = sampling_rate[0]
    if duration is None:
        # If failed to fetch the decoding information, decode the entire video.
    ...
    frames = None
    # If video stream was found, fetch video frames from the video.
    if container_x.streams.video and container_y.streams.video:
        video_frames_x, max_pts = pyav_decode_stream(
            container_x,
            video_start_pts,
            video_end_pts,
            container_x.streams.video[0],
            {"video": 0},
        )
        container_x.close()
        video_frames_y, max_pts = pyav_decode_stream(
            container_y,
            video_start_pts,
            video_end_pts,
            container_y.streams.video[0],
            {"video": 0},
        )
        container_y.close()
        frames_x = [frame.to_rgb().to_ndarray() for frame in video_frames_x]
        frames_x = torch.as_tensor(np.stack(frames_x))
        frames_y = [frame.to_rgb().to_ndarray() for frame in video_frames_y]
        frames_y = torch.as_tensor(np.stack(frames_y))
        frames = torch.cat([frames_x[0:1,...], frames_y[0:1,...], torch.abs(frames_x[0:1,...]-frames_y[0:1,...])], axis=0)      
    return frames, fps, decode_all_video
...
def decode(
    ...
    if backend == 'pyav_opt':
        container_x, container_y = container[0], container[1]
    ...
    elif backend == 'pyav_opt':
        assert (
            min_delta == -math.inf and max_delta == math.inf
        ), "delta sampling not supported in pyav"
        frames_decoded, fps, decode_all_video = pyav_decode_optical_flow(
            container_x,
            container_y,
            sampling_rate,
            num_frames,
