...
def attention_pool(tensor, pool, thw_shape, has_cls_embed=True, norm=None, tkl=20):
    if pool is None:
    ...
    if has_cls_embed:
        cls_tok, tensor = tensor[:, :, :tkl, :], tensor[:, :, tkl:, :]
    #print(tensor.shape)
...
def attention_pool_alternative(tensor, pool, thw_shape, thw_shape_new, has_cls_embed=True, norm=None):
    #if pool is None:
    #    return tensor, thw_shape
    tensor_dim = tensor.ndim
    if tensor_dim == 4:
    ...
    if has_cls_embed:
        cls_tok, tensor = tensor[:, :, :20, :], tensor[:, :, 20:, :]
    #print(tensor.shape)
    B, N, L, C = tensor.shape
    T, H, W = thw_shape
    #print(tensor.shape)
    ...
    tensor = torch.nn.functional.upsample(tensor, thw_shape_new)
    #print(tensor.size())
...
def cal_rel_pos_spatial(
    attn, q, k, has_cls_embed, q_shape, k_shape, rel_pos_h, rel_pos_w,tkl
):
    """
    Decomposed Spatial Relative Positional Embeddings.
    """
    sp_idx = tkl if has_cls_embed else 0
    q_t, q_h, q_w = q_shape
    k_t, k_h, k_w = k_shape
...
def cal_rel_pos_temporal(attn, q, has_cls_embed, q_shape, k_shape, rel_pos_t, tkl):
    """
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        elif mode in ['transpose_conv']:
            if pool_first:
                dim_conv = dim // num_heads if mode == "transpose_conv" else dim
            else:
                dim_conv = dim_out // num_heads if mode == "transpose_conv" else dim_out
            self.pool_q = (
    ...
    def forward(self, x, thw_shape,tkl=20):
        B, N, _ = x.shape
        if self.pool_first:
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        if mode != 'transpose_conv':
            ...
            #padding = [int(kv // 2) for kv in kernel_kv]
            self.pool_skip = None
            '''
            self.pool_skip = (
                torch.nn.ConvTranspose3d(
    ...
    def forward(self, x, thw_shape,tkl=20):
        x_norm = self.norm1(x)
        x_block, thw_shape_new = self.attn(x_norm, thw_shape,tkl)
        if self.dim_mul_in_att and self.dim != self.dim_out:
            x = self.proj(x_norm)
        if self.mode != 'transpose_conv':
            #    self.pool_skip = nn.Upsample(size=thw_shape_new)
            x_res, _ = attention_pool(
                x, self.pool_skip, thw_shape, has_cls_embed=self.has_cls_embed, tkl=tkl
            )
        else:
            x_res, _ = attention_pool_alternative(
            x, self.pool_skip, thw_shape,thw_shape_new, has_cls_embed=self.has_cls_embed)
...
class MultiScaleAttention_with_token_selection(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
    ...
    def forward(self, x, thw_shape):
        ...
        return x, thw_shape,q_shape, index, idx, cls_attn


class MultiScaleBlock_with_token_sleletion(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
        ...
        padding_skip = [int(skip // 2) for skip in kernel_skip]
        att_dim = dim_out if dim_mul_in_att else dim
        self.attn = MultiScaleAttention_with_token_selection(
            dim,
            att_dim,
            num_heads=num_heads,
            input_size=input_size,
        ...
        self.fuse_token = False
    def forward(self, x, thw_shape):
        x_norm = self.norm1(x)
        x_block, thw_shape_new,_, index, idx, cls_attn = self.attn(x_norm, thw_shape)
