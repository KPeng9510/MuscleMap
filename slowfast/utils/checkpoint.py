...
def load_checkpoint(
    ...
    """
    logger.info("Loading rgb network weights from {}.".format(path_to_checkpoint))
    path_to_checkpoint_diff = "/hkfs/work/workspace/scratch/fy2374-musclemap/checkpoints/checkpoints/split2_MViTv2_musclemap_RGB_DIFF_multitoken_reduction/checkpoints/checkpoint_epoch_00039.pyth"
    logger.info("Loading rgb diff network weights from {}.".format(path_to_checkpoint_diff))
    ...
    if convert_from_caffe2:
        ...
        with pathmgr.open(path_to_checkpoint_diff, "rb") as f:
            checkpoint_diff = torch.load(f, map_location="cpu")
        checkpoint_diff["model_state"] = normal_to_sub_bn(
            checkpoint_diff["model_state"], model_state_dict_3d
        )
        if inflation:
            ...
            if clear_name_pattern:
                for item in clear_name_pattern:
                    ...
                    model_state_dict_new_diff = OrderedDict()
                    for k in checkpoint_diff["model_state"]:
                        if item in k:
                            k_re = k.replace(
                                item, "", 1
                            )  # only repace first occurence of pattern
                            model_state_dict_new_diff[k_re] = checkpoint_diff[
                                "model_state"
                            ][k]
                            logger.info("renaming: {} -> {}".format(k, k_re))
                        else:
                            model_state_dict_new_diff[k] = checkpoint_diff["model_state"][
                                k
                            ]
                    checkpoint_diff["model_state"] = model_state_dict_new_diff
            pre_train_dict = checkpoint["model_state"]
            pre_train_dict_diff = checkpoint_diff["model_state"]
            ...
            for k, v in pre_train_dict_diff.items():
                k_new = k.replace(k.split('.')[0],k.split('.')[0]+'_diff')
                #print(k_new)
                if k_new in ['cls_token_diff','patch_embed_diff.proj.weight','patch_embed_diff.proj.bias']:
                    #print('found')
                    pre_train_dict_match[k_new]=v
                    continue
                
                if k_new in model_dict and v.size() == model_dict[k_new].size():
                    pre_train_dict_match[k_new]=v
            #print(pre_train_dict_match.keys())
            #import os
            #os.exit()
